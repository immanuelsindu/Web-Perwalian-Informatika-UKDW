            ...
            <div class="mx-md-5 mb-sm-5">
                <div id="cariCatatanPerwalian" class="row mt-md-5 mb-md-4 ml-sm-5">
                    <div class="col-md-7 col-sm-12 px-sm-0 mb-sm-2">
                        ...
                        <transition>
                            <div v-if="this.listCatatanArsip.length > 0" id="alertPencarian">
                                <p>{{ this.pesanError }}</p>
                                <p>
...
export default {
    ...
    methods: {
        ...
        async getTahunCatatanArsip() {
            try {
                const response = await axios.get(process.env.VUE_APP_API + `/getTahunCatatanArsipPerwalianDosen/`, {
                    params: {
        ...
        async filteredCatatanArsip() {
            try {
                const response = await axios.get(process.env.VUE_APP_API + `/filteredCatatanPerwalianDosenArsip/`, {
                    params: {
                        kode_dosen: this.kodeDosen,
        ...
        async hapusCatatanYa() {
            if (this.relasiCatatanHapus.tambah_peserta.length != 0) {
                for (let i = 0; i < this.relasiCatatanHapus.tambah_peserta.length; i++) {
                    ...
                    await axios.put(process.env.VUE_APP_API + `/deleteCatatanPerwalianDosen/`, paramObject);
                }
        ...
        async arsipCatatanYa() {
            if (this.relasiCatatanArsip.tambah_peserta != 0) {
                for (let i = 0; i < this.relasiCatatanArsip.tambah_peserta.length; i++) {
                    ...
                    await axios.put(process.env.VUE_APP_API + `/updateIsArsipCatatanPerwalianDosen/`, paramObject);
                }
