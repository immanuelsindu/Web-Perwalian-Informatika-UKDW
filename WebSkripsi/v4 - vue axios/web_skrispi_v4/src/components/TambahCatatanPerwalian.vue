...
export default {
    ...
    methods: {
        ...
        async searchNamaMahasiswa() {
            try {
                const response = await axios.get(process.env.VUE_APP_API + `/searchMahasiswa/`, {
                    params: {
                        kode_dosen: this.kodeDosen,
        ...
        async getMahasiswaPerwalianByTahun() {
            try {
                const response = await axios.get(process.env.VUE_APP_API + `/mahasiswaPerwalianPerTahun/`, {
                    params: {
                        kode_dosen: this.kodeDosen,
            ...
            if (!this.isFieldKosong()) { //mengecek apakah ada field yang kosong 
                ...
                if (this.wadahPesertaLainnya.length > 0) {
                    for (let i = 0; i < this.wadahPesertaLainnya.length; i++) {
                        ...
                        try {
                            const response = await axios.post(process.env.VUE_APP_API + `/tambahCatatanPerwalianDosen/`, tempParamObject);
                ...
                //=== Tambah Catatan YBS ====
                try {
                    const response = await axios.post(process.env.VUE_APP_API + `/tambahCatatanPerwalianDosen/`, paramObject);
                        ...
                        if (tempWadah != null) {
                            for (let i = 0; i < tempWadah.length; i++) {
                                try {
                                    ...
                                    await axios.put(process.env.VUE_APP_API + `/updateTambahPesertaCatatan/`, paramObject);
