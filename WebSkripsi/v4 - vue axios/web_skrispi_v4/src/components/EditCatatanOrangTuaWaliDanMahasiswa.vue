...
export default {
  ...
  methods: {
    async initData() {
      ...
      try {
        const response = await axios.get(
          process.env.VUE_APP_API + `/getCatatanPerwalianDosen/`,
          {
            params: {
          ...
          for (let i = 0; i < this.listMahasiswaPesertaLainnya.length; i++) {
            if (this.listMahasiswaPesertaLainnya[i].nim != this.nim) {
              if (
                ...
                try {
                  await axios.put(
                    process.env.VUE_APP_API + `/updateCatatanPerwalianDosen/`,
                    tempParamObject
                  );
                } catch (error) {
                  console.error(
              ...
              // jika peserta lain tidak punya id_catatan yang di insert sebelumnya , berarti dia peserta baru => insert data baru
                ...
                try {
                  const response = await axios.post(
                    process.env.VUE_APP_API + `/tambahCatatanPerwalianDosen/`,
                    tempParamObject
                  );
            ...
            if (isIdNol) {
              for (
                ...
                try {
                  ...
                  await axios.put(
                    process.env.VUE_APP_API + `/updateTambahPesertaCatatan/`,
                    paramObject
                  );
                } catch (error) {
                  console.error(
    ...
    async cariNamaMahasiswa() {
      try {
        const response = await axios.get(
          process.env.VUE_APP_API + `/searchMahasiswa/`,
          {
            params: {
              kode_dosen: this.kodeDosen,
    ...
    async hapusCatatanYa() {
      try {
        ...
        const response = await axios.put(
          process.env.VUE_APP_API + `/deleteCatatanPerwalianDosen/`,
          paramObject
        );
      ...
      if (
        ...
        for (let i = 0; i < daftarPesertaLainnya.length; i++) {
          ...
          try {
            ...
            await axios.put(
              process.env.VUE_APP_API + `/updateIsArsipCatatanPerwalianDosen/`,
              paramObject
            );
          } catch (error) {
            console.error("Terjadi kesalahan saat mengambil data:", error);
