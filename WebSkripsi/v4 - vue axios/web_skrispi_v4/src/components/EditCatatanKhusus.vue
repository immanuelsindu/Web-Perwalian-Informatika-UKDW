...
export default {
    ...
    methods: {
        async initData() {
            ...
            try {
                const response = await axios.get(process.env.VUE_APP_API + `/getCatatanDosen/`, {
                    params: {
        ...
        async simpan() {
            ...
            if (!this.isFieldKosong()) {
                ...
                try {
                    const response = await axios.put(process.env.VUE_APP_API + `/updateCatatanDosen/`, paramObject);
