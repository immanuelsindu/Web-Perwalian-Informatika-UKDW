                                ...
                                <div v-if="this.listCatatanGrupAngkatanTerbaru.length > 0" class="d-flex flex-row"
                                    ...
                                    <div v-for="(catatan, index) in this.listCatatanGrupAngkatanTerbaru" :key="index"
                                        ...
                                        <div id="judulCatatan">
                                            ...
                                            <p class="card-subtitle mb-2 text-muted">{{ this.convertTimestamp(catatan.waktu) }}
                                            </p>
...
export default {
    ...
    methods: {
        ...
        async getCatatanTerbaru() {
            ...
            try {
                const response = await axios.get(process.env.VUE_APP_API + `/getTahunCatatanAngkatanTerbaru/`, {
                    params: {
                        kode_dosen: this.kodeDosen,
                        tahun_angkatan: this.$route.params.id
                    },
        ...
        async initListMahasiswa() {
            ...
            try {
                ...
                const response = await axios.get(process.env.VUE_APP_API + `/mahasiswaPerwalianFiltered/`, {
                    params: {
                        kode_dosen: this.kodeDosen,
                        tahun_angkatan: this.$route.params.id,
        ...
        async getOpsiUrutanDetailAngkatan() {
            try {
                const response = await axios.get(process.env.VUE_APP_API + `/getOpsiDetailAngkatan/`, {
                    params: {
                        kode_dosen: this.kodeDosen,
                    },
                });
        ...
        async setOpsiUrutanBeranda() {
            try {
                ...
                const response = await axios.put(process.env.VUE_APP_API + `/setOpsiDetailAngkatan/`, paramObject);
        ...
        async simpan(loadValue) {
            if (this.opsiUrutanItemDetailAngkatan == "pilihan1") {
                ...
                this.items[0].data.forEach(dataPosisi => {
                    console.log(dataPosisi.id);
                    console.log(dataPosisi.pos);
