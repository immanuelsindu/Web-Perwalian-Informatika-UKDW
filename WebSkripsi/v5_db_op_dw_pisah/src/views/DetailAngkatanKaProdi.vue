                ...
                <div id="infoMahasiswaBimbingan" class="d-flex flex-row flex-wrap col-12">
                  ...
                  <div v-for="items in this.jumlahStatusPerwalian.get(this.$route.params.id)" :key="items"
                    ...
                    <div class="d-flex align-items-center col-12" @click="this.toggleItemSelection(items.status)">
                      ...
                      <div class="col-1 text-right p-0 d-flex justify-content-center">
                        <p class="m-0 jumlahMahasiswaBimbingan">{{ items.jumlah }}</p>
                      </div>
                    </div>
                  </div>
                </div>
...
export default {
  ...
  methods: {
    ...
    async getJumlahStatusMahasiswaAktifDanTidakAktif() {
      let total = 0;
      this.jumlahStatusPerwalian.get(this.$route.params.id).forEach(item => {
        if (item.status === 'AK' || item.status === 'TA') {
          total += parseInt(item.jumlah);
        }
      });
      this.jumlahAktifDanTidakAktif = total
    },
    async getJumlahStatusMahasiswa() {
      ...
      try {
        ...
        if (response.data.error === false) {
          this.jumlahStatusPerwalian.set(this.$route.params.id, response.data.response)
          ...
          //menghitung jumlah status "Aktif +dan Tidak Aktif"
          this.getJumlahStatusMahasiswaAktifDanTidakAktif()
    ...
    hitungJumlahMahasiswa(list) {
      return list.reduce((total, item) => total + parseInt(item.jumlah), 0);
    },
    scrollTop() {
      window.scrollTo({
        top: 0,
    ...
    async initListMahasiswa() {
      ...
      try {
        ...
        const response = await axios.get(process.env.VUE_APP_API_DATAWAREHOUSE + `/getMahasiswaAngkatanFilteredKaProdi/`, {
          params: {
            tahun_angkatan: this.$route.params.id,
