                        ...
                        <div id="buttonBatalSimpan" class="mt-3">
                            <div class="row justify-content-between">
                                ...
                                <div class="col-xl-6 col-sm-6 d-flex justify-content-end mb-5">
                                    <v-btn id="buttonCetakPresensi" class="mr-2" prepend-icon="mdi-file-export" color="#0277BD" :disabled="this.tombolTidakAktifPDF" type="submit" @click="this.simpanKePDF()">Cetak Presensi PDF</v-btn>
...
export default {
 ...
 + `/getCatatanPerwalianDosen/`, {
        ...
        simpanKePDF(){
            this.tombolTidakAktifPDF = true
            const doc = new jsPDF();

            // Menambahkan judul
            doc.setFont("helvetica", "bold")
            doc.text('Presensi Mahasiswa', 10, 10)
            doc.setFontSize(13)
            doc.setFont("helvetica", "normal")
            doc.text(this.judul + " " + this.waktu + " "+ this.waktuAwal, 10,18);
            // doc.text(this.agendaPerwalian, 10, 26)

            // Tentukan lebar maksimum untuk setiap baris (misalnya, 100)
            const maxWidth = 200;
            // Tentukan posisi awal
            let yPosition = 30;
            // Bagi teks ke dalam beberapa baris
            const splitText = doc.splitTextToSize(this.agendaPerwalian, maxWidth);
            // Tulis setiap baris teks
            splitText.forEach(line => {
                doc.text(line, 10, yPosition);
                
                // Tingkatkan posisi untuk baris berikutnya
                yPosition += 8; // Sesuaikan sesuai kebutuhan Anda
            });

            // Mengatur header tabel
            const headers = [['NIM', 'Nama', 'Status Presensi', 'Keterangan']];

            // Mengatur data tabel
            const data = this.listPresensi.map(row => [row.nim, row.nama, row.status, row.keterangan === '' ? '-': row.keterangan]);

            // Membuat tabel menggunakan jsPDF
            autoTable(doc, {
                head: headers,
            ...
            let judulCatatan = this.tahunAngkatan+"_"+this.judul+ "_"+this.waktu+"_"+this.getWaktuSekarangTime()
            doc.save(judulCatatan);
        }
    },
    mounted() {
